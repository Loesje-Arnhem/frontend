  ...
  <search role="search">
    <form method="get" class="form" @keyup.esc="reset" @submit.prevent="submit">
      ...
      <div class="input-wrapper">
        <form-input-text id="search" type="search" name="search" autocomplete="off" :model-value="modelValue"
          class="search" :title="$t('search')" v-bind="$attrs" @input="input" @keyup-down="onArrowDown"
          @keyup-up="onArrowUp" />
        <div class="buttons">
          <button v-if="modelValue.length" type="button" class="btn-reset" @click="reset">
        ...
        <div v-if="resultsWithHighlightText.length && showList" class="autocomplete">
          <ul class="list">
            <li v-for="(result, index) in resultsWithHighlightText" :key="result.id">
              <nuxt-link :class="{ active: index === arrowCounter }" class="btn-result" :to="result.uri"
                v-html="result.title" />
            </li>
          </ul>
        </div>
