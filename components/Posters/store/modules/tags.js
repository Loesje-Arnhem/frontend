...
const getters = {
  ...
  selectedSources: state => state.selectedTags.filter(tag => tag.type === 'source'),
  selectedSubjects: state => state.selectedTags.filter(tag => tag.type === 'subject'),
};
...
const actions = {
  populateTags: ({ commit }) => {
    const getSources = axios.get('source', {
      params: {
        per_page: 100,
      },
    });

    const getSubjects = axios.get('subject', {
      params: {
        per_page: 100,
      },
    });

    const addTypeOnTags = (tags, type) => tags.map((tag) => {
      const newTag = tag;
      newTag.type = type;
      return newTag;
    });
